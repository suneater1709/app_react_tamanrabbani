...
export default function Admission() {
    ...
    useEffect(() => {
        Promise.all([
            cmsApi.getPrograms().catch(() => ({ success: false, data: [] })),
    ...
    useEffect(() => {
        const handlePopState = (e: PopStateEvent) => {
            ...
            } else if (e.state && e.state.showForm === false) {
                setShowForm(false);
                setStep(1);
            } else {
                setShowForm(false);
    ...
    const handleBackToGuide = () => {
        if (window.history.state?.showForm) {
            window.history.back();
        } else {
            setShowForm(false);
            setStep(1);
    ...
    const onSubmitFinal = async () => {
        ...
        try {
            const res = await axios.post('/api/v1/public/admissions', payload, {
    ...
    // MODE 1: Informasi PPDB (Gambar 5)
    if (!showForm) {
        return (
            <div className="py-12 bg-slate-50 min-h-screen text-left">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="text-center max-w-3xl mx-auto mb-12">
                        <span className="inline-block px-3 py-1 bg-pink-50 text-pink-600 text-xxs font-extrabold uppercase tracking-widest rounded mb-3">
                            ALUR & SYARAT PPDB
                        </span>
                        <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-800">Informasi Pendaftaran Online</h1>
                        <p className="mt-3 text-slate-500 text-xs sm:text-sm">
                            Panduan lengkap jadwal pendaftaran, persyaratan berkas fisik, serta infografis pendaftaran dari rumah.
                        </p>
                    </div>

                    {/* Info Grid (Gambar 5 Layout) */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch max-w-5xl mx-auto mb-16">
                        
                        {/* A. Jadwal Gelombang */}
                        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-md border-none flex flex-col justify-start">
                            ...
                            <div className="space-y-4">
                                {(settings?.ppdb_waves && settings.ppdb_waves.length > 0 ? settings.ppdb_waves : [
                                    ...
                                    <div 
                                        key={wave.id || idx} 
                                        className={`p-4 rounded-2xl border-none transition-all ${
                                            wave.is_active 
                                                ? 'bg-teal-50/70 ring-1 ring-teal-600/30 shadow-xs' 
                                                : 'bg-slate-50/60'
                                        }`}
                                    >
                                        <div className="flex justify-between items-center gap-2">
                                            <h4 className="font-bold text-slate-850 text-xs sm:text-sm">{wave.name}</h4>
                                            {wave.badge && (
                                                <span className="text-[9px] bg-yellow-400 text-slate-900 font-bold px-2 py-0.5 rounded-full uppercase tracking-wider animate-pulse shrink-0">
                                                    {wave.badge}
                                                </span>
                                            )}
                                        </div>
                                        <p className="text-xs text-slate-500 mt-1">{wave.period}</p>
                                        {wave.note && (
                                            <span className={`text-[10px] block mt-1.5 font-semibold ${
                                                wave.is_active ? 'text-teal-700' : 'text-slate-400'
                                            }`}>
                                                {wave.note}
                                            </span>
                                        )}
                        ...
                        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-md border-none flex flex-col justify-start">
                            <h3 className="text-base sm:text-lg font-bold text-slate-800 flex items-center gap-2 mb-4">
                                <FileText className="text-teal-650" size={20} />
                                <span>Syarat & Ketentuan Dokumen</span>
                            </h3>

                            <ul className="space-y-3.5 text-xs sm:text-sm text-slate-600">
                                <li className="flex items-start gap-2">
                                    <span className="text-teal-500 mt-0.5">•</span>
                                    <span><strong>Usia Minimal:</strong> KB (Minimal 3 tahun per Juli 2026), TK A (Minimal 4 tahun), TK B (Minimal 5 tahun).</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="text-teal-500 mt-0.5">•</span>
                                    <span>Scan/Foto <strong>Akta Kelahiran Siswa</strong> (Asli & Jelas).</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="text-teal-500 mt-0.5">•</span>
                                    <span>Scan/Foto <strong>Kartu Keluarga (KK)</strong> terbaru.</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="text-teal-500 mt-0.5">•</span>
                                    <span>File pasfoto berwarna <strong>Anak yang Ceria</strong> ukuran 3x4 (setengah badan).</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="text-teal-500 mt-0.5">•</span>
                                    <span>Membayar Biaya Form Pendaftaran sebesar <strong>{settings?.ppdb_form_fee || 'Rp 100.000'}</strong> (ditransfer atau diserahkan saat verifikasi berkas).</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl p-8 sm:p-10 shadow-md max-w-5xl mx-auto text-center mb-10 border-none">
                        ...
                        <h3 className="text-xl sm:text-2xl font-bold text-slate-800 mb-10">4 Langkah Mudah Daftar dari Rumah</h3>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 text-left">
                            {[
                                { step: 1, title: 'Isi Formulir Online', desc: 'Klik tombol daftar dan isi form 5 tahap biodata ananda serta orang tua secara jujur.' },
                                { step: 2, title: 'Unggah Dokumen', desc: 'Unggah file foto akta lahir, KK, dan foto ananda secara langsung dari HP Anda.' },
                                { step: 3, title: 'Simpan Kode Reg', desc: 'Simpan nomor registrasi unik Anda untuk melacak status seleksi panitia PPDB.' },
                                { step: 4, title: 'Selesai & Registrasi', desc: 'Ketika status lolos/diterima, datang ke sekolah untuk mengukur seragam sekolah.' }
                            ].map((item) => (
                                <div key={item.step} className="space-y-3 relative group">
                                    <div className="w-10 h-10 rounded-full bg-teal-50 flex items-center justify-center font-extrabold text-teal-700 text-lg shadow-xs group-hover:bg-teal-650 group-hover:text-white transition-all duration-300">
                                        {item.step}
                                    </div>
                        ...
                        <div className="mt-12 flex justify-center border-t border-slate-100 pt-8">
                            <button
                                ...
                                className="px-8 py-4 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-full transition-all duration-300 shadow-md hover:shadow-lg flex items-center gap-2 text-sm md:text-base cursor-pointer transform hover:-translate-y-0.5"
                            >
                                <span>Mulai Daftar Online Sekarang</span>
                                <ArrowRight size={18} />
                            </button>
                        </div>
                    </div>
        ...
        <div className="py-12 bg-slate-50 text-left min-h-screen">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center max-w-3xl mx-auto mb-10">
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-800">Formulir PPDB Online</h1>
                        ...
                        <div className="border-t border-slate-100 pt-6 flex justify-between gap-4">
                            {step > 1 ? (
                                ...
                                <button
                                    ...
                                    className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-xs transition flex items-center gap-1.5 border-none cursor-pointer"
                                >
                                    <ChevronLeft size={16} /> Kembali ke Panduan
                                </button>
                            )}
